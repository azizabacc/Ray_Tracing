...
-graphical_display : ( string ) local, mean, multipath"""
        ...
        for receiver in self.receiver_list:
            for transmitter in self.transmitter_list:
                ...
                self.ray_tracing([], max_number_reflection, transmitter, receiver, self.wall_list, multipath_list)
                if "mean" in graphical_display:
                    receiver.received_mean_power += calculate_mean_power(multipath_list, receiver, transmitter)
                if "local" in graphical_display:
                    receiver.received_local_power += calculate_local_power(multipath_list, receiver, transmitter)
                if ("multipath" in graphical_display) and (receiver.position == receiver_position) and (
                        transmitter == self.transmitter_list[0]):
                    ray_graphical_display(receiver, transmitter, self.wall_list, multipath_list)
            if "local" in graphical_display:
                ...
                data_local.append([receiver.position[0], receiver.position[1], receiver.received_bit_rate])
            if "mean" in graphical_display:
                ...
                date_mean.append([receiver.position[0],receiver.position[1], receiver.received_bit_rate])

        # self.ray_graphical_display(receiver,transmitter,multipath_list)#todo
        if "local" in graphical_display:
            power_graphical_display(data_local, self.wall_list, self.transmitter_list)
        if "mean" in graphical_display:
            power_graphical_display(date_mean,self.wall_list, self.transmitter_list)
...
-graphical_display : ( string ) local, mean, rays"""
        ...
        for receiver in self.receiver_list:
            ...
            if "local" in graphical_display:
                # finalization of the calculation of the local power
                end_calculated_localpower(receiver)
                # conversion of the local power to bit rate
                receiver.received_bit_rate = bit_rate_power(receiver.received_local_power)  # todo
                # receiver.received_bit_rate = receiver.received_local_power
                data_local.append([receiver.position[0],receiver.position[1], receiver.received_bit_rate])
            if "mean" in graphical_display:
                # conversion of the mean power to bit rate
                receiver.received_bit_rate = bit_rate_power(receiver.received_mean_power)  # todo
                # receiver.received_bit_rate = receiver.received_mean_power
                data_mean.append([receiver.position[0],receiver.position[1], receiver.received_bit_rate])

        # self.ray_graphical_display(receiver,transmitter,ray_list)#todo
        if "local" in graphical_display:
            x = [i[0] for i in data_local]
            y = [i[1] for i in data_local]
            val = [i[2] for i in data_local]

        if "mean" in graphical_display:
            x = [i[0] for i in data_mean]
            y = [i[1] for i in data_mean]
            val = [i[2] for i in data_mean]
