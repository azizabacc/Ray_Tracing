from excel_to_wall import create_wall, plot_wall
...
-graphical_display : ( string ) local, mean, rays"""
        ...
        for receiver in self.receiver_list:
            for transmitter in self.transmitter_list:
                ...
                if "mean" in graphical_display:
                    receiver.received_mean_power += self.calculate_mean_power(ray_list, receiver, transmitter)
                if "local" in graphical_display:
                    receiver.received_local_power += self.calculate_local_power(ray_list, receiver, transmitter)
                if ("ray" in graphical_display) and (receiver.position == receiver_position) and (
                        transmitter == self.transmitter_list[0]):
                    self.ray_graphical_display(receiver, transmitter, ray_list)
            if "local" in graphical_display:
                # finalization of the calculation of the local power
                self.end_calculated_localpower(receiver)
                # convertion of the local power to bit rate
                receiver.received_bit_rate = self.bit_rate_power(receiver.received_local_power)#todo
            ...
            if "mean" in graphical_display:
                ...
                receiver.received_bit_rate = self.bit_rate_power(receiver.received_mean_power)#todo
        ...
        if "local" in graphical_display:
            self.power_graphical_display("local_bit_rate.txt")
        if "mean" in graphical_display:
            self.power_graphical_display("mean_bit_rate.txt")
...
"""
    ...
    def ray_creation(self, sub_wall_list, transmitter, receiver):
        ...
        for wall in sub_wall_list:
            # images creation
            image_point = self.image(point, wall)
        ...
        if len(sub_wall_list) != 0:
            ray.distance = self.dist(receiver.position, image_list[
        ...
        for j in range(len(image_list)):
            ...
            intersection_point = ray_line.intersection(sub_wall_list[len(image_list) - 1 - j])  # intersection point wall / ray
            ...
            if sub_wall_list[len(image_list) - 1 - j].point_in_Line_outof_wall(intersection_point):
                ...
                break
            if self.between(intersection_point, ray_point, image_list[len(image_list) - 1 - j]):
            ...
            self.reflection_coeff_calc(sub_wall_list[len(image_list) - 1 - j], ray, ray_line)
        ...
        if len(ray.point_list) != 0:
            self.verif_transmission(ray, self.wall_list, sub_wall_list)
            ...
            for j in wall_list:
                ...
                if not j.point_in_Line_outof_wall(intersection):
                    ...
                    if (intersection == ray.point_list[i]) or (intersection == ray.point_list[i + 1]):
                        ray.point_list = []
                        return 0
                    if self.between(intersection, ray.point_list[i], ray.point_list[i + 1]):
                        """ if the intersection with the wall j takes place => transmission coefficient calculation """
                        self.transmission_coeff_calc(j, ray, ray_line)
        return 0
    ...
    def calculate_local_power(self, ray_list, receiver, transmitter):
    ...
    """def ray_graphical_display(self, receiver, transmitter, ray_list):
        ...
        for ray in ray_list:
            self.plott(ray.point_list)
        plot_wall(self.wall_list)
